...
def branch_exists(branch):
    code = subprocess.run(f"git rev-parse --verify {branch}",
                          shell=True, executable='/bin/bash').returncode
    return code == 0
...
def open_url(url):
    if platform.system() == "Darwin":
        subprocess.run(f'open "{url}"', shell=True, executable='/bin/bash')
    else:
        print(f"\nOpen this PR URL manually: {url}")
...
def main():
    ...
    subprocess.run("git rev-parse --is-inside-work-tree", shell=True, check=True, executable='/bin/bash')
